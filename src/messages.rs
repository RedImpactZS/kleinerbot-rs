            ...
            match &event {
                ...
                Event::MessageCreate(msg) => {
                    ...
                    for attach in &message.attachments {
                        ...
                        if !response.status().is_success() {
                            return Err(anyhow!("Web request failed {}", response.status()));
                        }
                    ...
                    if !images.is_empty() {
                        if cattaches.len() >= cattaches.capacity() {
                            debug!("Stack is filled, draining first");
                            cattaches.drain(0..1);
                        }
                        debug!(
                            "Caching attachment {}/{}",
                ...
                Event::MessageUpdate(msg) => {
                    ...
                    let oldmsg = cache
                        ...
                        .await?
                        .context("Message cache miss")?;

                    let gchannel = cache
                        .guild_channel(oldmsg.channel_id)
                        .await?
                        .context("Channel cache miss")?;
                    ...
                    match gchannel.as_ref() {
                        GuildChannel::Text(ref c) => {
                            let author = msg
                                .author
                                .clone()
                                .context("UPD: Failed to get cached author")?;

                            let avatar = &author
                                .avatar
                                .to_owned()
                                .context("UPD: Failed to get avatar")?;

                            let newcontent =
                                &msg.content.to_owned().context("UPD: Failed to content")?;

                            let timestamp = &msg
                                .timestamp
                                .to_owned()
                                .context("UPD: Failed to get timestamp")?;
                ...
                Event::MessageDelete(msg) => {
                    ...
                    let gchannel = cache
                        .guild_channel(oldmsg.channel_id)
                        .await?
                        .context("DEL:Failed to get cached channel")?;
                    ...
                    match gchannel.as_ref() {
                        GuildChannel::Text(ref c) => {
                            let author = cache
                                ...
                                .await?
                                .context("DEL:Failed to get cached author")?;
